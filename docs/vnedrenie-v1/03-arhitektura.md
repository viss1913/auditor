...
- Развёртывание: отдельный процесс на сервере с **GPU не менее 16 ГБ VRAM** (базовый ориентир для выбранной модели и запаса под пиковую нагрузку); для работы модуля ИИ **GPU обязателен**. Доступ только из внутренней сети, **без выхода в публичный интернет** для обработки данных Заказчика.
